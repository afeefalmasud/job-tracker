...
function toggleStyle(id){
    ...
    allBtn.classList.remove('bg-[#3B82F6]', 'bg-[#10B981]', 'bg-[#EF4444]', 'text-white');
    interviewBtn.classList.remove('bg-[#3B82F6]', 'bg-[#10B981]', 'bg-[#EF4444]', 'text-white');
    rejectedBtn.classList.remove('bg-[#3B82F6]', 'bg-[#10B981]', 'bg-[#EF4444]', 'text-white');
...
mainContainer.addEventListener('click', function(event){
    ...
    const allCard = Array.from(allCards.children).find(
        card => card.querySelector('.title').innerText === parentNode.querySelector('.title').innerText
    );
    ...
    if(event.target.classList.contains('inter-btn')){
        ...
        parentNode.querySelector('.apply').classList.add('bg-[#10B981]', 'text-white');
        parentNode.querySelector('.apply').classList.remove('bg-[#EF4444]','bg-[#EEF4FF]');
        ...
        parentNode.querySelector('.apply').classList.add('bg-[#EF4444]', 'text-white');
        parentNode.querySelector('.apply').classList.remove('bg-[#10B981]','bg-[#EEF4FF]');
        ...
        const cardExist = rejectedList.find(item => item.title === cardInfo.title);
        if(!cardExist){
...
function renderInterview(){
    ...
    for(let interview of interviewList){
        ...
        div.className = 'my-5 p-6 bg-[#ffffff] border-2 border-[#dfdfdf] border-l-4 border-l-[#10B981] rounded-2xl';
        div.innerHTML = `
...
function renderRejected(){
    ...
    for(let rejected of rejectedList){
        let div = document.createElement('div');
        div.className = 'my-5 p-6 bg-[#ffffff] border-2 border-[#dfdfdf] border-l-4 border-l-[#EF4444] rounded-2xl';
        div.innerHTML = `
            <div class="flex justify-between">
